...
io.write(ws.filename('cells', postfix='i'), cells_data)
...
source = ws.source('cells', postfix='i');
header = ', '.join([h[0] for h in source.dtype.names]);
np.savetxt(ws.filename('cells', postfix='i', extension='csv'), source[:], header=header, delimiter=',', fmt='%s')
